...
Come abbiamo discusso, un segnale analogico misurato in un dato istante, ad esempio ￼, può, in linea di principio, assumere qualsiasi valore. Tuttavia, abbiamo a disposizione solo un numero finito di bit e quindi un numero finito di valori diversi per rappresentare una misurazione in un computer. Di conseguenza, dobbiamo mappare questo numero infinito di valori su un numero finito di valori. Questo processo è chiamato quantizzazione. Per eseguire la quantizzazione, che viene effettuata da un quantizzatore, supponiamo innanzitutto che il nostro segnale misurato, ￼, che tipicamente rappresenta una tensione, sia in un determinato intervallo, ad esempio da ￼ a ￼. Se abbiamo a disposizione ￼ bit, possiamo rappresentare ￼ valori diversi. Questi potrebbero, ad esempio, rappresentare i numeri interi da 0 a ￼ o da ￼ a ￼. Se dividiamo l’intervallo tra ￼ e ￼ in ￼ celle di dimensione uguale, le celle devono avere dimensione:

￼

Questo processo è chiamato quantizzazione uniforme ed è la forma di quantizzazione più comune per l’audio. La dimensione del passo, ￼, ovviamente diminuisce all’aumentare del numero di bit, ￼. Il numero di bit usati per campione nell’audio è tipicamente di 16, 24 o 32 bit. Va notato che quando il segnale in ingresso è al di fuori dell’intervallo specificato da ￼ a ￼, si verifica solitamente un fenomeno di clipping, un problema noto alla maggior parte delle persone.
