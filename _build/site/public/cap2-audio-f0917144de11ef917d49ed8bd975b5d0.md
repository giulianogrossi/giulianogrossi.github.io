...
Se i segnali che prendiamo in considerazione hanno ampiezze comprese tra $-V/2$ e $V/2$, questo può essere ottenuto dividendo l'insieme $\left[-V/2,V/2\right]$ in $l$ intervalli, detti `livelli`, ed attribuendo ad un punto $x\in\left[-V/2,V/2\right]$ il centro del livello in cui $x(n)$ cade.  Detti $\{x_1,\dots,x_l\}$ i centri dei vari livelli, l'operazione di quantizzazione può essere allora descritta dalla funzione $Q$ che ad ogni $x(n)$ associa il centro più vicino:
