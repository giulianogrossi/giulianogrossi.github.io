...
Se i segnali che prendiamo in considerazione hanno ampiezze comprese $-V/2$ tra e $V/2$, questo può essere ottenuto dividendo l'insieme $\left[-V/2,V/2\right]$ in $l$ intervalli, detti liv`elli, ed attribuendo ad un punto $x\in\left[-V/2,V/2\right]$ il centro del livello in cui $x$ cade.  Detti $\{x_1,\dots,x_l\}$ i centri dei vari livelli, l'operazione di quantizzazione può essere allora descritta dalla funzione $Q$ che ad ogni $x$ associa il centro più vicino:
$$
