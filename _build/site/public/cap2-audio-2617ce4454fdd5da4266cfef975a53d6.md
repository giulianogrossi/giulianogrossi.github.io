...
Un modo semplice di quantizzare consiste nel prefissare un un insieme finito di $l$ valori numerici $\{x_1,\dots,x_l\}$ e di associare ad ogni numero $x$ il valore numerico $x_k$ che è più vicino a $x$. Il passo ulteriore è quello della \emph{codifica} dei valori dell'insieme $\{x_1,\dots,x_l\}$ in parole  binarie
...
Per eseguire la quantizzazione, che viene effettuata da un quantizzatore, supponiamo innanzitutto che il segnale misurato, \( x(n) \), che tipicamente rappresenta una tensione, sia in un determinato intervallo, ad esempio da \( -V/2 \) a \( V/2 \). Se abbiamo a disposizione \( q \) bit, possiamo rappresentare \( 2^{q} \) valori diversi. Questi potrebbero, ad esempio, rappresentare i numeri interi da \( 0 \) a \( 2^{q} - 1 \) o da \( -2^{q - 1} \) a \( 2^{q - 1} - 1 \).
