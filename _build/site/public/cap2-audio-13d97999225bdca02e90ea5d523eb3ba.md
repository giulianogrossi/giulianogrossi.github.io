...
Ora che sappiamo quale dovrebbe essere la dimensione del passo del quantizzatore, dati l'intervallo e il numero di bit, vediamo a quale funzione viene effettuata dal quantizzatore uniforme per mappare i valori in input . La mappatura di $x(n)$ nei livelli passo $\Delta$ viene effettuata da un quantizzatore, che denotiamo con $Q(\cdot)$ e il cui output indichiamo come $ y_n $, ossia:
