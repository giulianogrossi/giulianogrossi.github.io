...
Una volta fissata la dimensione del passo di quantizzazione $\Delta$, dati l'intervallo $V$ e il numero di bit $M$, vediamo una possibile funzione implementata dal quantizzatore uniforme per mappare i valori in input $x(n)$ nei valori in output $y(n)$, come descritto da {eq} e ne studiamo brevemente l'errore commesso.. Se 
$$
\mathcal{Q}[x(n)]=  \quad 
$$
