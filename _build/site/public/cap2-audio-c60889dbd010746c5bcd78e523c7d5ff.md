...
Un modo semplice di quantizzare consiste nel prefissare un un insieme finito di $l$ valori numerici $\{x_1,\dots,x_l\}$ e di associare ad ogni valore misurato, \( x(n) \), che tipicamente rappresenta una tensione, sia in un determinato intervallo, ad esempio da \( -V/2 \) a \( V/2 \). il valore numerico $x_k$ che è più vicino a $x$. Il passo ulteriore è quello della codifica dei valori dell'insieme $\{x_1,\dots,x_l\}$ in parole  binarie opportunamente codificate
