...
1. Codifica Binaria e Livelli di Tensione

	•	In un sistema digitale a $q$ bit, ogni configurazione dei bit rappresenta un valore discreto, compreso tra $0$ e $2^{q}-1$.
	•	Ciascuna combinazione di bit è convertita in un livello di tensione corrispondente. Ad esempio, un DAC a 8 bit può rappresentare ￼ livelli distinti, mentre un DAC a 12 bit può rappresentare ￼ livelli, migliorando la precisione della rappresentazione analogica.

2. Passo di Quantizzazione

	•	L’intervallo di tensione totale del DAC è definito dai valori di tensione ￼ e ￼, dove ￼ è il livello di tensione più basso (di solito 0 V) e ￼ il livello più alto.
	•	Il passo di quantizzazione ￼ è la differenza di tensione tra due livelli consecutivi e può essere calcolato come:
￼
	•	Ogni valore binario in ingresso corrisponde a un livello discreto di tensione, con il valore più alto pari a ￼ e il più basso a ￼.

3. Conversione Binario-Tensione

	•	Per ottenere la tensione in uscita ￼ per una configurazione di bit specifica ￼ (un valore binario tra 0 e ￼), la formula è:
￼
	•	Qui, ￼ rappresenta il valore binario in ingresso, e moltiplicandolo per il passo di quantizzazione, otteniamo la tensione corrispondente. Ad esempio, in un DAC con ￼ V e ￼ V, per un ingresso di ￼ bit, si avrebbero 8 livelli di tensione, e ogni livello incrementerebbe di ￼ V.

4. Schema di Funzionamento

	•	Step 1: Il DAC riceve un numero binario come ingresso, ad esempio 011 per un DAC a 3 bit.
	•	Step 2: Calcola il livello di tensione corrispondente moltiplicando il valore decimale del numero binario (in questo caso 3) per il passo di quantizzazione.
	•	Step 3: Genera la tensione di uscita equivalente, che verrà usata come segnale analogico.

5. Precisione e Risoluzione

	•	La precisione del DAC è legata al numero di bit e alla stabilità dei livelli di tensione. Più bit ha il DAC, più livelli di tensione può rappresentare e quindi maggiore è la risoluzione, riducendo la differenza tra livelli di quantizzazione (￼) e rendendo la rappresentazione analogica più fedele all’originale digitale.
