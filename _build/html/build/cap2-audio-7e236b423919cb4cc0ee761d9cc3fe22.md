...
- L’intervallo di tensione totale del DAC è definito dai valori di tensione $V_{\text{min}}$ e $V_{\text{max}}$, dove $V_{\text{min}}$ è il livello di tensione più basso (di solito 0 V) e ￼ il livello più alto.
- Il passo di quantizzazione $V_{\text{min}}$ è la differenza di tensione tra due livelli consecutivi e può essere calcolato come:
￼
