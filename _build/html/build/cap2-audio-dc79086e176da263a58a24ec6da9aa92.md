...
Una volta fissata la dimensione del passo di quantizzazione \( \Delta \), l'intervallo \( V \) e il numero di bit \( M \), possiamo definire una funzione che il quantizzatore uniforme applica per mappare i valori di ingresso \( x(n) \) ai valori di uscita \( y(n) \). In un quantizzatore uniforme, l’intervallo di quantizzazione viene diviso in \( 2^M \) livelli distinti. Ogni livello rappresenta un possibile valore di output, in modo che il segnale \( x(n) \) sia mappato al livello \( y(n) \) più vicino, secondo la seguente espressione:

\[
y(n) = Q(x(n)) = \Delta \cdot \text{round}\left(\frac{x(n)}{\Delta}\right),
\]
...
L’errore di quantizzazione, denotato come \( e(n) \), è dato dalla differenza tra il segnale originale \( x(n) \) e il segnale quantizzato \( y(n) \):
