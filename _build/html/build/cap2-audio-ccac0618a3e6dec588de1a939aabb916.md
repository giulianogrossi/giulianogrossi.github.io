...
$$
Q(x)=\underset{x_i\in \{x_1,\dots,x_l\}}{\arg\min}\abs{x-x_i}.
$$
...
Per eseguire la quantizzazione, che viene effettuata da un quantizzatore, supponiamo innanzitutto che il nostro segnale misurato, \( x_n \), che tipicamente rappresenta una tensione, sia in un determinato intervallo, ad esempio da \( -\alpha \) a \( \alpha \). Se abbiamo a disposizione \( \beta \) bit, possiamo rappresentare \( 2^{\beta} \) valori diversi. Questi potrebbero, ad esempio, rappresentare i numeri interi da \( 0 \) a \( 2^{\beta} - 1 \) o da \( -2^{\beta - 1} \) a \( 2^{\beta - 1} - 1 \).

Se dividiamo l'intervallo tra \( -\alpha \) e \( \alpha \) in \( 2^{\beta} \) celle di dimensione uguale, le celle devono avere dimensione:

\[
\Delta = \frac{\alpha}{2^{\beta - 1}}.
\]

Questo processo è chiamato quantizzazione uniforme ed è la forma di quantizzazione più comune per l'audio. La dimensione del passo, \( \Delta \), ovviamente diminuisce all'aumentare del numero di bit, \( \beta \). Il numero di bit usati per campione nell'audio è tipicamente di 16, 24 o 32 bit.
