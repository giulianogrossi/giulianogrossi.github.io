...
Una volta fissata la dimensione del passo del quantizzatore $\Delta$, dati l'intervallo e il numero di bit, vediamo a quale funzione viene effettuata dal quantizzatore uniforme per mappare i valori in input $x(n)$ nei valori in output $y(n)$ caratterizzati dai livelli determinati dal passo , ossia:
$$
\mathcal{Q}[x(n)]=  \quad 
$$
