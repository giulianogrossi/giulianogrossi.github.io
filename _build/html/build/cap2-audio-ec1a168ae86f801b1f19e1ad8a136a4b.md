...
dove \( \text{round}(\cdot) \) indica l’arrotondamento al numero intero più vicino. Qui, ogni valore \( x(n) \) viene quindi mappato a un intervallo di ampiezza \( \Delta \) nell’output \( y(n) \).

L'errore di quantizzazione, \( e_n \), è la differenza tra l'ingresso e l'uscita, ovvero:
...
Questo è illustrato nella figura precedente  in cui l'errore di quantizzazione per un quantizzatore
uniforme di due bit (quattro livelli).. La quantità di rumore è quindi determinata dalla dimensione del passo, \( \Delta \). Più piccolo è \( \Delta \), minore sarà il rumore aggiunto al segnale. Infatti, possiamo osservare che gli errori di quantizzazione si trovano tra \( -\frac{1}{2} \) e \( \frac{1}{2} \), a causa dell'arrotondamento in (3.11).

Nella Fig. 3.9 è mostrato un istogramma degli errori di quantizzazione per 10.000 valori di ingresso compresi tra \( -1 \) e \( 1 \) con una dimensione del passo di \( \Delta = 0.125 \). Come si può vedere, i diversi errori si verificano con frequenza approssimativamente uniforme e gli errori sono compresi, come previsto, tra \( -\frac{1}{2} \) e \( \frac{1}{2} \).

Per analizzare ulteriormente l'effetto della quantizzazione, dobbiamo introdurre un nuovo concetto, ovvero la potenza di un segnale. Possiamo considerare il nostro segnale come una rappresentazione digitale di un segnale di tensione che è stato campionato. Analogamente, possiamo considerare il nostro segnale \( x_n \) come una rappresentazione del segnale di tensione \( v(t) \) che appare all'uscita di un convertitore digitale-analogico. Se colleghiamo poi l'uscita del DAC a una resistenza di valore \( R = 1 \), la potenza istantanea dissipata nella resistenza è semplicemente \( v^2(t) \) (vedi Fig. 3.10). La potenza media su un intervallo di tempo \( T \) è data da
