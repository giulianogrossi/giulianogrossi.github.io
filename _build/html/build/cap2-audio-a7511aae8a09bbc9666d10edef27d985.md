...
livelli di uguale ampiezza $V/l$ è detto `quantizzatore uniforme` e i livelli hanno dimensione:
\[
\Delta=\frac{V}{l}=x_i-x_{i+1} ,\quad \text{con}\quad 1\le i\le l-1,
\]
anche chiamato `passo di quantizzazione`.

Se abbiamo a disposizione \( m \) bit, possiamo rappresentare \( l=2^{m} \) livelli i cui centri , gli elementi $\{x_1,\dots,x_l\}$ possono essere codificati
con parole di$m$ bit:
\[
