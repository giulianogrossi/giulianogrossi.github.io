...
Per eseguire la quantizzazione, che viene effettuata da un quantizzatore, supponiamo innanzitutto che il segnale misurato, \( x(n) \), che tipicamente rappresenta una tensione, sia in un determinato intervallo, ad esempio da \( -V/2 \) a \( \alpha \). Se abbiamo a disposizione \( \beta \) bit, possiamo rappresentare \( 2^{\beta} \) valori diversi. Questi potrebbero, ad esempio, rappresentare i numeri interi da \( 0 \) a \( 2^{\beta} - 1 \) o da \( -2^{\beta - 1} \) a \( 2^{\beta - 1} - 1 \).
