...
Una volta fissata la dimensione del passo del quantizzatore $\Delta$, dati l'intervallo e il numero di bit, vediamo una possibile funzione implementata dal quantizzatore uniforme per mappare i valori in input $x(n)$ nei valori in output $y(n)$
$$
\mathcal{Q}[x(n)]=  \quad 
$$
