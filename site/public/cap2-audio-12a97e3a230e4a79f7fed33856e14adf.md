...
Un sistema quantizzatore in cui l'intervallo $[-V/2,V/2]$ è suddiviso in $l$
livelli di uguale ampiezza $V/l$ è detto `quantizzatore uniforme` e le celle di dimensione uguale, le celle devono avere dimensione:
\[
\Delta=\frac{V}{l}=x_i-x_{i+1} ,\quad \text{con}\quad 1\le i\le l-1,
\]
è chiamato \emph{passo di quantizzazione}.

Se $l=2^m$, gli elementi $\{x_1,\dots,x_l\}$ possono essere codificati
con parole di$m$ bit:
\[
x_i=b_{i1}\cdots b_{im},\quad \text{con}\quad b_{ik}\in\{0,1\}\;\;(1\le i\le l).
\]
Il sistema in questo caso è detto \emph{quantizzatore uniforme} 

Se abbiamo a disposizione \( q \) bit, possiamo rappresentare \( 2^{q} \) valori diversi. Questi potrebbero, ad esempio, rappresentare i numeri interi da \( 0 \) a \( 2^{q} - 1 \) o da \( -2^{q - 1} \) a \( 2^{q - 1} - 1 \).
