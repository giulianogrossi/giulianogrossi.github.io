...
Per analizzare ulteriormente l'effetto della quantizzazione, occorre richiamare il concetto di potenza di un segnale, che per un segnale digitale è la somma
...
Ipotizziamo che il segnale di riferimento per il calcolo della potenza sia il segnale sinusoidale analogico $x(t)=A\sin(\omega\, t)$, la cui potenza è del tutto analoga al segnale discreto $x(n)=A\sin(\omega\, nT_s)$. La media di tale segnale è 0, poiché:

$$
\lim_{T\rightarrow \infty}\frac{\int_{-T}^{T}A \sin t\,dt}{2T}=0.
$$

La varianza o potenza del segnale $\sigma_s^2$ di tale segnale è invece $A^2$. Infatti per $T$ grande si ha:

$$
\sigma^2=\frac{\int_{-T}^{T}(A\sin(\omega\, t)-0)^2\,
dt}{2T}=\frac{\frac{A^2}{8}\int_{-T}^{T}(1+\cos 2t)\,dt}{2T}=\frac{A^2}{8}.
$$


$$
\sigma^2=\lim_{T\rightarrow \infty}\frac{\int_{-T}^{T}(\frac{A}{2}\sin t-0)^2\,
dt}{2T}=\lim_{T\rightarrow \infty}\frac{\frac{A^2}{8}\int_{-T}^{T}(1+\cos 2t)\,dt}{2T}=\frac{A^2}{8}.
$$

In tal caso:

$$
\text{SQNR}=10\log_{10}\frac{A^2/8}{\Delta^2/12}=10\log_{10}\frac{V^2}{\Delta^2}\frac{A^2}{V^2}+10\log_{10}\f{3}{2}=20\log_{10}l\frac{A}{V}+1.76 \;\;\text{dB}.
$$
